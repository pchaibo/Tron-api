...
class WithdrawalController extends BaseController
{
    ...
    public function index()
    {
        ...
        //查询统计
        $field = array('SUM(tkmoney)'=>'tkmoney','SUM(sxfmoney)'=>'sxfmoney','SUM(money)'=>'money','count(id)'=> 'num');
        $sum = M('tklist')->where($where)->field($field)->find();
        $this->assign('sumtkmoney',$sum);
        //print_r($sum);
        C('TOKEN_ON',false);
        $this->display();
    }
    ...
    public function exportorder()
    {
        ...
        foreach ($data as $item){
            ...
            $list[] = array(
                ...
                'tongdao'=>getProduct($item['payapiid']),
                'status'=>$status,
            );
        }
        ...
        if(IS_POST){
            ...
            if($id){
                ...
                if ($status == 2) {
                    ...
                    M('Member')->where(['id'=>$userid])->save(['balance'=>array('exp',"balance+{$tkmoney}")]);
                }
                $res = $Tklist->where("id=" . $id)->save($data);
                $this->ajaxReturn(['status'=>$res]);
            }else{
