    ...
    public function checkLogin()
    {
        if(IS_POST){
            ...
            if(!$verify->check($verification)){
                $this->ajaxReturn(['errorno'=>1,'msg'=>'账号或密码输入错误！','url'=>'']);
            }
            $membermodel = M('Member');
            //$info = $membermodel->where(['username'=>$username,'password'=>md5($loginpassword.C('DATA_AUTH_KEY'))])->find();
            $info = $membermodel->where(['username'=>$username,])->find();
            if($info && $info['groupid']<4){
                $admin_indo = [
                    'uid'=>$info['id'],
                ...
                $this->ajaxReturn(['errorno'=>1,'msg'=>'你的账号类型不能在此登录！','url'=>U('User/Login/index')]);
            }
